...
export const createSpace = async (spaceName: string, spaceTemplateURL: string, spaceReason: string) => {
...
export const setMap = async (spaceURL: string, mapID: string, newMapData: Partial<GameMap>) => {
   const urlParser = spaceURL.split('/')
   const { status, data } = await axios({
      method: 'GET',
      url: `https://api.gather.town/api/v2/spaces/${encodeURIComponent(
